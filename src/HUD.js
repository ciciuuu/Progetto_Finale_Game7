...
function create_hud(s) {
    // Creo un layer con Z-Index altissimo (1000) per stare sopra a tutto il gioco
    livello_HUD = PP.layers.create(s)
    PP.layers.set_z_index(livello_HUD, 1000)

    // EFFETTI DANNO (Vignette)
    // Iniziano invisibili (alpha 0) e appaiono quando colpiti
    ...
    // [PHASER] Fisso alla camera per non muoversi col giocatore
    ...
    // Pezzi singoli (Inizialmente invisibili, alpha 0)
    ...
    // Pistola fissa (inquinante) per inizio gioco quando non hai scelta
    ...
    // Barra Vita (Un frame per ogni stato di salute)
...
function update_hud(s, player) {
    ...
    // 1. GESTIONE GRAFICA COLLEZIONABILI
    // Controllo quali pezzi ho raccolto e accendo (alpha=1) la parte corrispondente
    // La logica cambia leggermente in base al livello (base vs base_3)
    if (typeof is_collezionabile_preso === "function") {
        
        if (s.scene.key === "base") {
            // Livello 1: ID standard
    ...
    if (is_arma_sbloccata) {
        ...
        if (PP.interactive.kb.is_key_down(s, PP.key_codes.L)) {
            if (hud_tasto_R_rilasciato) {
                ...
                hud_tasto_R_rilasciato = false // Blocco input
            }
        } else {
            hud_tasto_R_rilasciato = true // Sblocco input
        }

        // Cambio animazione pistola in base alla modalità
        let anim_pistola_target = hud_modalita_inquinante ? "anim_inquinante" : "anim_normale"
        // [PHASER] Controllo animazione corrente per non riavviarla
        ...
        // Se non ho ancora l'arma, mostro quella fissa (solo inquinante)
        pistola_fissa.visibility.hidden = false
        pistola.visibility.hidden = true
        hud_modalita_inquinante = true
    }

    // 3. GESTIONE BARRA VITA
    let vita_raw = PP.game_state.get_variable("HP_player")
    
    if (vita_raw !== undefined && vita_raw !== null) {
        // Arrotondo il numero per sicurezza: se è 4.5 diventa 5, se è "4" diventa 4
        // Questo evita che cerchi animazioni inesistenti come "health_4.5"
